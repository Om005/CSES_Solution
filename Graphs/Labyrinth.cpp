...
void solve() {
    ...
    vector<vector<pair<ll, ll>>> papa(n, vector<pair<ll, ll>>(m, {-1, -1}));
    queue<vll> q;
    ...
    while(q.size()) {
        ...
        for(ll i=0;i<si;i++) {
            auto it = q.front();
            q.pop();
            papa[it[0]][it[1]] = {it[2], it[3]};
            for(ll k=0;k<4;k++) {
                ll newx = it[0]+dir[k], newy = it[1]+dir[k+1];
                if(newx>=0 && newy>=0 && newx<n && newy<m && !vis[newx][newy] && s[newx][newy]!='#') {
                    vis[newx][newy] = true;
                    q.push({newx, newy, it[0], it[1]});
                }
            }
        }
    }
    if(papa[st.first][st.second].first == -1) cout << "NO";
    else{
        string ans = "";
        ...
        while(papa[st.first][st.second].first != -1) {
            ll px = papa[st.first][st.second].first;
            ll py = papa[st.first][st.second].second;
            ll dx = st.first - px;
            ll dy = st.second - py;
            ans += mp[{dx, dy}];
            st = {px, py};
