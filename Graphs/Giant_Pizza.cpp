...
void solve() {
    i2(m, n);
    vector<vector<ll>> graph(2*n+1, vll());
    vector<vector<ll>> rgraph(2*n+1, vll());
    for(ll i=0;i<m;i++) {
        char c1, c2;
        ll a, b;
        cin >> c1 >> a >> c2 >> b;
        ...
        graph[na].emplace_back(b);
        graph[nb].emplace_back(a);
        rgraph[a].emplace_back(nb);
        rgraph[b].emplace_back(na);
    }
    vector<ll> comp(2*n+1);
    ll curr = 1;
    vector<ll> vis(2*n+1, false);
    stack<ll> st;
