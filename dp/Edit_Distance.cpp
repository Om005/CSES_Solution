...
ll help(string &s, string &t, ll i, ll j, vector<vll> &dp) {
    if(i<0 || j<0) {
        return max(i, j)+1;
    }
    if(dp[i][j]!=-1) return dp[i][j];
    ll a = 1+help(s, t, i-1, j, dp);
    ll b = 1+help(s, t, i, j-1, dp);
    ll c = !(s[i]==t[j])+help(s, t, i-1, j-1, dp);
