...
void solve() {
    ...
    vector<vector<ll>> up(n+1, vll(20));
    vector<ll> len(n+1);
    ...
    function<ll(ll, ll)> jump = [&](ll node, ll k) -> ll {
        if(k < 0) return -1;
        for(ll i=0;i<20;i++){
    ...
    for(ll i=0;i<m;i++){
        i2(a, b);
        ll entry = jump(a, len[a]);
        if(jump(a, len[a]-len[b]) == b){
            cout << len[a]-len[b] << nl;
        }
        else if(jump(entry, len[entry]-len[b]) == b) {
            cout << len[entry]-len[b]+len[a] << nl;
        }
        else cout << -1 << nl;
    }
}
