...
ll gm;
ll help(vll &v, ll i, ll lst, vector<vll> &dp) {
    if(i<0) return 1LL;
    if(dp[i][lst+1]!=-1) return dp[i][lst+1];
    if(v[i]!=0) {
        if(lst!=-1 && abs(v[i]-lst) > 1) return dp[i][lst+1] = 0LL;
        else return dp[i][lst+1] = help(v, i-1, v[i], dp);
    }
    ll res = 0LL;
    if(lst==-1) {
        for(ll j=1;j<=gm;j++){
            res = (res+help(v, i-1, j, dp))%MODE;
        }
    }
    else {
        for(ll j=-1;j<=1;j++) {
            if(lst+j>=1 && lst+j<=gm)
            res = (res+help(v, i-1, lst+j, dp))%MODE;
        }
    }
    return dp[i][lst+1] = res;
}
...
void solve() {
    ...
    gm = m;
    vin(n, v);
    vector<vll> dp(n, vll(m+2, -1));
    ll ans = help(v, n-1, -1, dp);
    cout << ans;
