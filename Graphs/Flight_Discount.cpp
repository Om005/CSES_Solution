...
void solve() {
    ...
    vector<vector<pair<ll, ll>>> graph2(n+1, vector<pair<ll, ll>>());
    vector<vll> edges(m, vll(3));
    for(ll i=0;i<m;i++){
        i3(a, b, c);
        edges[i] = {a, b, c};
        graph[a].emplace_back(b, c);
        graph2[b].emplace_back(a, c);
    }
    vll dis1(n+1, LLONG_MAX);
    vll dis2(n+1, LLONG_MAX);
    dis1[1] = 0LL;
    dis2[n] = 0LL;
    ...
    while(q.size()){
        auto it = q.top();
        q.pop();
        ll node = it.second, curr = it.first;
        if(dis1[node] < curr) continue;
        for(auto &child: graph[node]) {
            if(dis1[child.first] > child.second+curr) {
                dis1[child.first] = child.second+curr;
                q.push({dis1[child.first], child.first});
            }
        }
    }
    q.push({0, n});
    while(q.size()){
        ...
        q.pop();
        ll node = it.second, curr = it.first;
        if(dis2[node] < curr) continue;
        for(auto &child: graph2[node]) {
            if(dis2[child.first] > child.second+curr) {
                dis2[child.first] = child.second+curr;
                q.push({dis2[child.first], child.first});
            }
        }
    }
