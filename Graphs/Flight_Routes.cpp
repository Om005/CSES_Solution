...
void solve() {
    ...
    while(q.size()){
        ...
        if (dis[node].size() == k && curr > *prev(dis[node].end())) continue;
        for(auto &child: graph[node]) {
            ll childn = child.first;
            ll total = curr+child.second;
            if(dis[childn].size()<k){
                dis[childn].insert(total);
                q.push({total, childn});
            }
            else {
                auto pt = prev(dis[childn].end());
                if (*pt > total) {
                    dis[childn].erase(pt);
                    dis[childn].insert(total);
                    q.push({total, childn});
