...
void solve() {
    ...
    vector<vll> dis(n, vll(n, INT_MAX));
    vector<pair<ll, ll>> dir = {{-2, 1}, {-2, -1}, {2, 1}, {2, -1}, {-1, 2}, {-1, -2}, {1, 2}, {1, -2}};
    dis[0][0] = 0;
    q.push({0, {0, 0}});
    while(q.size()) {
        auto it = q.top();
        q.pop();
        ll x = it.second.first, y = it.second.second, curr = it.first;
        for(auto &k: dir) {
            ll newx = x+k.first, newy = y+k.second;
            if(newx>=0 && newy>=0 && newx<n && newy<n && curr+1 < dis[newx][newy]) {
                dis[newx][newy] = curr+1;
                q.push({curr+1, {newx, newy}});
            }
        }
    }
    for(ll i=0;i<n;i++){
