...
void solve() {
    ...
    vector<pair<pair<ll, ll>, ll>> v2(n);
    for(ll i=0;i<n;i++){
        cin >> v1[i].first.first >> v1[i].first.second;
        v1[i].second = i;
        v2[i] = v1[i];
    }
    sort(v1.begin(), v1.end(), [&](pair<pair<ll, ll>, ll> &a, pair<pair<ll, ll>, ll> &b) {
        if(a.first.first==b.first.first) return a.first.second>b.first.second;
        return a.first.first<b.first.first;
    });
    sort(v2.begin(), v2.end(), [&](pair<pair<ll, ll>, ll> &a, pair<pair<ll, ll>, ll> &b) {
        if(a.first.second==b.first.second) return a.first.first>b.first.first;
        return a.first.second<b.first.second;
    });
    vll maxi(n+1);
    for(ll i=1;i<=n;i++){
        maxi[i] = max(maxi[i-1], v2[i-1].first.first);
    }
    vll ans1(n);
    vll ans2(n);
    ll curr = 0;
    for(ll i=0;i<n;i++){
        if(curr >= v1[i].first.second) ans2[v1[i].second] = 1;
        curr = max(curr, v1[i].first.second);
    }
    for(ll i=n-1;i>=0;i--){
        if(maxi[i] >= v2[i].first.first) ans1[v2[i].second] = 1;
    }
