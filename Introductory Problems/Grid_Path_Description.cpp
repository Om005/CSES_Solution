...
void help(int i, int j, int curr) {
    if (curr == 48) {
        if (i == N - 1 && j == 0) ans++;
        return;
    }
    ...
    if ((!check(i - 1, j) || vis[i - 1][j]) && (!check(i + 1, j) || vis[i + 1][j])) {
        if (check(i, j - 1) && check(i, j + 1) && !vis[i][j - 1] && !vis[i][j + 1]) return;
    }

    if ((!check(i, j - 1) || vis[i][j - 1]) && (!check(i, j + 1) || vis[i][j + 1])) {
        if (check(i - 1, j) && check(i + 1, j) && !vis[i - 1][j] && !vis[i + 1][j]) return;
    }

    vis[i][j] = true;

    if (s[curr] == '?' || s[curr] == 'L') {
        if (check(i, j - 1) && !vis[i][j - 1]) help(i, j - 1, curr + 1);
    }
    if (s[curr] == '?' || s[curr] == 'R') {
        if (check(i, j + 1) && !vis[i][j + 1]) help(i, j + 1, curr + 1);
    }
    if (s[curr] == '?' || s[curr] == 'U') {
        if (check(i - 1, j) && !vis[i - 1][j]) help(i - 1, j, curr + 1);
    }
    if (s[curr] == '?' || s[curr] == 'D') {
        if (check(i + 1, j) && !vis[i + 1][j]) help(i + 1, j, curr + 1);
    }
