...
void solve() {
    ...
    function<ll(ll i, ll col, ll dia1, ll dia2)> help = [&](ll i, ll col, ll dia1, ll dia2) -> ll {
        if(i==8) return 1LL;
        ll res = 0LL;
        for(ll j=0;j<8;j++) {
            if(!((col>>j)&1) && !((dia1>>(i+j))&1) && !((dia2>>(8-i+j))&1) && s[i][j]!='*') {
                res += help(i+1, col|(1<<j), dia1|(1<<(i+j)), dia2|(1<<(8-i+j)));
            }
        }
        return res;
    };
